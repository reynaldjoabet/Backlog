...
object AesApp extends IOApp.Simple {
  ...
  val run: IO[Unit] =
    for {
      ...
      ecbEncryptedBytes =
        Stream(message).through(utf8.encode).covary[IO].through(AES.ECB.PKCS5Padding.encrypt(key))
      ...
      ecbDecryptedMessage <-
        ecbDecryptedBytes.through(utf8.decode).compile.toList.map(_.mkString(""))
      ...
      cbcDecryptedMessage <-
        cbcDecryptedBytes.through(utf8.decode).compile.toList.map(_.mkString(""))

      data = message.getBytes(UTF_8)

      javaEcbEncryptedBytes = AES.ECB.PKCS5Padding.javaEncrypt(key.getEncoded, data)
      javaEcbDecryptedBytes =
        AES.ECB.PKCS5Padding.javaDecrypt(key.getEncoded, javaEcbEncryptedBytes)
      javaEcbEncryptedHex =
        Stream.chunk(Chunk.array(javaEcbEncryptedBytes)).through(hex.encode).toList.mkString("")
      javaEcbDecryptedMessage = new String(javaEcbDecryptedBytes, UTF_8)

      javaCbcEncryptedBytes = AES
                                .CBC
                                .PKCS5Padding
                                .javaEncrypt(
                                  key.getEncoded,
      ...
      javaCbcEncryptedHex =
        Stream.chunk(Chunk.array(javaCbcEncryptedBytes)).through(hex.encode).toList.mkString("")
