...
object AesApp extends IOApp.Simple {
  ...
  val aesAlgo = "AES"
  def keySpec(key: Array[Byte]): IO[SecretKey] = IO(
    new SecretKeySpec(key, aesAlgo)
  )
  ...
  object AES {
    ...
    object ECB {
      ...
      object PKCS5Padding {
        ...
        private[this] def crypto[F[_]: Sync](
    ...
    object CBC {
      ...
      object PKCS5Padding {
        ...
        private[this] def ivpsOutput(
            ivpsChunk: Chunk[Byte],
            outputChunk: Chunk[Byte]
        ): (IvParameterSpec, Chunk[Byte]) = {
          val nextIv: Array[Byte] = ivpsChunk.takeRight(blockSize).toArray
          (new IvParameterSpec(nextIv), outputChunk)
        }
        private[this] def encrypt[F[_]: Sync](
            transformation: String,
            key: SecretKey
        ): (IvParameterSpec, Chunk[Byte]) => F[(IvParameterSpec, Chunk[Byte])] =
          (ivps, input) =>
            crypto(transformation, Cipher.ENCRYPT_MODE, key)
              .apply(ivps, input)
              .map(output => ivpsOutput(output, output))

        private[this] def decrypt[F[_]: Sync](
            transformation: String,
            key: SecretKey
        ): (IvParameterSpec, Chunk[Byte]) => F[(IvParameterSpec, Chunk[Byte])] =
          (ivps, input) =>
            crypto(transformation, Cipher.DECRYPT_MODE, key)
              .apply(ivps, input)
        ...
        def encrypt[F[_]: Sync](
          ...
          _.chunkTimesN(blockSize)
            .evalScanChunksLast[F, Byte, Byte, IvParameterSpec](ivps) {
              encrypt(noPaddingTransformation, key)
            } { (ivps, c) =>
              encrypt(pkcs5PaddingTransformation, key).apply(ivps, c).map(_._2)
            }
        }
        ...
        def decrypt[F[_]: Sync](
          ...
          _.chunkTimesN(blockSize)
            .evalScanChunksLast[F, Byte, Byte, IvParameterSpec](ivps) {
              ...
              decrypt(pkcs5PaddingTransformation, key).apply(ivps, c).map(_._2)
            }
  ...
  val run: IO[Unit] =
    for {
      ...
      ecbEncryptedBytes = Stream(message)
        .through(utf8.encode)
        .covary[IO]
        .through(AES.ECB.PKCS5Padding.encrypt(key))
      ecbDecryptedBytes = ecbEncryptedBytes.through(
        AES.ECB.PKCS5Padding.decrypt(key)
      )
      ecbEncryptedHex <- ecbEncryptedBytes
        .through(hex.encode)
        .compile
        .toList
        .map(_.mkString(""))
      ecbDecryptedMessage <- ecbDecryptedBytes
        .through(utf8.decode)
        .compile
        .toList
        .map(_.mkString(""))

      cbcEncryptedBytes = Stream(message)
        .through(utf8.encode)
        .covary[IO]
        .through(AES.CBC.PKCS5Padding.encrypt(key, ivps))
      cbcDecryptedBytes = cbcEncryptedBytes.through(
        AES.CBC.PKCS5Padding.decrypt(key, ivps)
      )
      cbcEncryptedHex <- cbcEncryptedBytes
        .through(hex.encode)
        .compile
        .toList
        .map(_.mkString(""))
      cbcDecryptedMessage <- cbcDecryptedBytes
        .through(utf8.decode)
        .compile
        .toList
        .map(_.mkString(""))
      ...
      javaEcbEncryptedBytes = AES.ECB.PKCS5Padding
        .javaEncrypt(key.getEncoded, data)
      javaEcbDecryptedBytes = AES.ECB.PKCS5Padding
        .javaDecrypt(key.getEncoded, javaEcbEncryptedBytes)
      javaEcbEncryptedHex = Stream
        .chunk(Chunk.array(javaEcbEncryptedBytes))
        .through(hex.encode)
        .toList
        .mkString("")
      javaEcbDecryptedMessage = new String(javaEcbDecryptedBytes, UTF_8)

      javaCbcEncryptedBytes = AES.CBC.PKCS5Padding.javaEncrypt(
        key.getEncoded,
        ivps.getIV,
        data
      )
      javaCbcDecryptedBytes = AES.CBC.PKCS5Padding.javaDecrypt(
        key.getEncoded,
        ivps.getIV,
        javaCbcEncryptedBytes
      )
      javaCbcEncryptedHex = Stream
        .chunk(Chunk.array(javaCbcEncryptedBytes))
        .through(hex.encode)
        .toList
        .mkString("")
      ...
      _ <- IO.println(
        s"EcbEncrypted Correct    : ${Set(ecbEncryptedHex, javaEcbEncryptedHex).size == 1}"
      )
      _ <- IO.println(
        s"CbcEncrypted Correct    : ${Set(cbcEncryptedHex, javaCbcEncryptedHex).size == 1}"
      )
      ...
      _ <- IO.println(
        s"EcbDecrypted Correct    : ${Set(message, ecbDecryptedMessage, javaEcbDecryptedMessage).size == 1}"
      )
      _ <- IO.println(
        s"CbcDecrypted Correct    : ${Set(message, cbcDecryptedMessage, javaCbcDecryptedMessage).size == 1}"
      )
    } yield ()
}
