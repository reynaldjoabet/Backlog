...
object CertApp extends IOApp.Simple {
  ...
  val message: Array[Byte] =
    "Hello, use X.509 cert!".getBytes(StandardCharsets.UTF_8)

  def loadKeyStore(keyStoreFile: String, password: String): IO[KeyStore] =
    Resource
      .fromAutoCloseable(
        IO.blocking(getClass.getResourceAsStream(keyStoreFile))
      )
      .use { inputStream =>
        IO.blocking {
  ...
  def encrypt(certificate: Certificate, message: Array[Byte]): IO[Array[Byte]] =
    IO {
      val cipher: Cipher =
        Cipher.getInstance(certificate.getPublicKey.getAlgorithm)
